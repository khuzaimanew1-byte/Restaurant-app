...
export class EmailService {
  ...
  async sendOtp(to: string, otp: string, purpose: "login" | "reset"): Promise<void> {
    ...
    const subtitle = isReset ? "Password reset code" : "Admin login code";

    try {
      await this.transporter.sendMail({
        from: `"MyRestaurant" <${from}>`,
        to, subject,
        headers: { "X-Mailer": "MyRestaurant-Auth/1.0", "X-Priority": "1", "Importance": "high" },
        text: `MyRestaurant — ${subtitle}\n\nYour one-time code is: ${otp}\n\nThis code is valid for 10 minutes. Do not share it with anyone.\n\nIf you did not request this code, you can safely ignore this email.\n`,
        html: `
        <div style="font-family:Arial,Helvetica,sans-serif;max-width:420px;margin:0 auto;padding:0;">
          <div style="background:#20242b;border-radius:12px;padding:32px;">
            ...
            <p style="color:#9aa3b0;margin:0 0 28px;font-size:13px;">${subtitle}</p>
      ...
      if (msg.includes("535") || msg.includes("BadCredentials") || msg.includes("Username and Password"))
        throw new HttpException("Email sending failed: Gmail App Password is incorrect or expired.", HttpStatus.SERVICE_UNAVAILABLE);
      if (msg.includes("Missing credentials"))
        throw new HttpException("Email service misconfigured: GMAIL or GMAIL_APP_PASSWORD env var is missing.", HttpStatus.SERVICE_UNAVAILABLE);
      if (msg.includes("ECONNREFUSED") || msg.includes("ETIMEDOUT") || msg.includes("ENOTFOUND"))
        throw new HttpException("Email sending failed: Cannot reach Gmail servers.", HttpStatus.SERVICE_UNAVAILABLE);
      throw new HttpException(`Email sending failed: ${msg}`, HttpStatus.SERVICE_UNAVAILABLE);
    }
  }
}
