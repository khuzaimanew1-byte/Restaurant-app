...
const CSS = `
  ...
  *, *::before, *::after { box-sizing:border-box; margin:0; padding:0 }
  ...
  /* ── Initial page load ── */
  @keyframes top-in  { from{opacity:0;transform:translateY(-14px)} to{opacity:1;transform:translateY(0)} }
  .top-enter { animation:top-in .5s cubic-bezier(.22,1,.36,1) both }

  /* ── Page transition — illustration ──
     direction="next":  out slides LEFT  | in slides from RIGHT
     direction="prev":  out slides RIGHT | in slides from LEFT
  ── */
  @keyframes illus-exit-left  {
    from { opacity:1; transform:translateX(0)   scale(1)    }
    to   { opacity:0; transform:translateX(-9%) scale(.93)  }
  }
  @keyframes illus-exit-right {
    from { opacity:1; transform:translateX(0)   scale(1)    }
    to   { opacity:0; transform:translateX(9%)  scale(.93)  }
  }
  @keyframes illus-enter-right {
    from { opacity:0; transform:translateX(12%) scale(.93)  }
  ...
  .illus-exit-left   { animation:illus-exit-left   .32s cubic-bezier(.4,0,1,1) both }
  .illus-exit-right  { animation:illus-exit-right  .32s cubic-bezier(.4,0,1,1) both }
  .illus-enter-right { animation:illus-enter-right .46s cubic-bezier(.22,1,.36,1) .05s both }
  .illus-enter-left  { animation:illus-enter-left  .46s cubic-bezier(.22,1,.36,1) .05s both }

  /* ── Page transition — text block ── */
  @keyframes text-exit-up   { from{opacity:1;transform:translateY(0)} to{opacity:0;transform:translateY(-16px)} }
  @keyframes text-exit-down { from{opacity:1;transform:translateY(0)} to{opacity:0;transform:translateY(16px)}  }
  @keyframes text-enter-up  {
    from{opacity:0;transform:translateY(22px)}
    to  {opacity:1;transform:translateY(0)}
  }
  @keyframes text-enter-down{
    from{opacity:0;transform:translateY(-22px)}
    to  {opacity:1;transform:translateY(0)}
  }

  .text-exit-up    { animation:text-exit-up   .28s cubic-bezier(.4,0,1,1)     both }
  .text-exit-down  { animation:text-exit-down .28s cubic-bezier(.4,0,1,1)     both }
  .text-enter-up   { animation:text-enter-up  .44s cubic-bezier(.22,1,.36,1) .1s both }
  .text-enter-down { animation:text-enter-down .44s cubic-bezier(.22,1,.36,1) .1s both }

  /* ── Staggered headline / desc / dots ── */
  .stagger-1 { animation-delay:.06s  }
  .stagger-2 { animation-delay:.14s  }
  .stagger-3 { animation-delay:.22s  }

  /* ── Dot transitions ── */
  .dot { transition:width .36s cubic-bezier(.22,1,.36,1), background .28s }
`;
...
function AttendanceIllus({ dark }: { dark: boolean }) {
  return (
    <div style={{ position: "relative", width: SIDE, height: SIDE }}>
      ...
      {/* Center card */}
      <div style={{
        position: "absolute", width: "36%", height: "48%",
        top: "26%", left: "32%",
        ...
        <div style={{ width: "72%", display: "flex", flexDirection: "column", gap: 4 }}>
          {[1, .7, .86].map((w, i) => (
            <div key={i} style={{
              height: 3, borderRadius: 2, width: `${w * 100}%`,
        ...
        <div style={{
          ...
          fontSize: "clamp(7px,1.8vw,9px)", fontWeight: 700, letterSpacing: .6,
...
function Chip({ dark, val, sub, an, delay, col, t, l, r, b }: {
...
/* ─── Pages ──────────────────────────────────────────────────────── */
const PAGES = [
  { headline: "Attendance,\nSimplified",    desc: "Track attendance securely through office Wi‑Fi with offline support and automatic synchronization.", Illus: AttendanceIllus },
  { headline: "Manage Leave\nEffortlessly", desc: "Apply for leave, track requests, and receive approvals through a professional digital workflow.",      Illus: LeaveIllus      },
  { headline: "Know Your\nProgress",        desc: "Monitor attendance score, punctuality, working hours, and performance trends in one place.",           Illus: AnalyticsIllus  },
];
...
export function OnboardingFlow() {
  ...
  const [next, setNext]       = useState(0);       // page being navigated TO
  ...
  /* Navigate: start exit → swap → enter → idle */
  const goTo = useCallback((target: number) => {
    if (phase !== "idle" || target === idx) return;
    const d = target > idx ? "fwd" : "bwd";
    setDir(d);
    setNext(target);
    setPhase("exit");
    ...
    // After exit animation (300ms) → swap page + start enter
    exitTimer.current = setTimeout(() => {
      setIdx(target);
      setPhase("enter");
      // After enter animation (500ms) → idle
      enterTimer.current = setTimeout(() => setPhase("idle"), 500);
    }, 300);
  }, [phase, idx]);

  const isLast = idx === PAGES.length - 1;

  /* Which CSS class to apply based on phase + direction */
  const illusClass = () => {
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{
          ...
          {/* Keyed so React remounts on page change, restarting all float/pulse animations */}
        ...
        <div style={{
          ...
          <h1 key={`h-${idx}`} className={textClass("stagger-1")} style={{
            ...
            lineHeight: 1.13, letterSpacing: "-.03em",
